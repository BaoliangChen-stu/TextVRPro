...
if params['version'] == 'offline':
    ...
    class ZYZOCRanns_datasets(TextVideoDataset):
        ...
        def ocr_format(self, sample):
            ...
            sample['ocr_info'] = dict(gt_ocr=gt_text_list, gt_pos=np.array(gt_pos_list).astype(np.float32).tolist(),
                                      valid_l=valid_l)
            return sample
    ...
    class ZYZOCRanns_datasets(TextVideoDataset):
        ...
        def ocr_format(self, sample):
            ...
            if valid_l > self.topl:
                ...
                gt_text_list = gt_text_list + [' '] * (self.topl - valid_l)
                gt_pos_list = gt_pos_list + [np.zeros(12)] * (self.topl - valid_l)
            sample['ocr_info'] = dict(gt_ocr=gt_text_list, gt_pos=np.array(gt_pos_list).astype(np.float32),
                                      valid_l=valid_l)
            return sample
