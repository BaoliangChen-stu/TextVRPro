...
if version == 'mmspacetimetransformer':
    class VarAttention(nn.Module):
        ...
        def forward(self, x, einops_from, einops_to, **einops_dims):
            ...
            (cls_q, ocr_q, q_), (cls_k, ocr_k, k_), (cls_v, ocr_v, v_) = map(
                lambda t: (t[:, 0:sp1], t[:, sp1:sp2], t[:, sp2:]), (q, k, v))

            # let CLS token attend to key / values of all patches across time and space
            cls_out = attn(torch.cat([cls_q, ocr_q], 1), k, v)
            ...
            # # expand ocr token keys and values across time or space and concat
            # r = q_.shape[0] // ocr_k.shape[0]
            # ocr_k, ocr_v = map(lambda t: repeat(t, 'b o d -> (b r) o d', r=r), (ocr_k, ocr_v))
            #
            # k_ = torch.cat((ocr_k, k_), dim=1)
            # v_ = torch.cat((ocr_v, v_), dim=1)
    ...
    class SpaceTimeTransformer(nn.Module):
        ...
        def forward_features(self, x, ocr_feats):
            b, curr_frames, channels, _, _ = x.shape
            x = self.patch_embed(x)
            x = x.flatten(2).transpose(2, 1)
            x = x.reshape(b, -1, self.patch_embed.embed_dim)  # [b, txl, c]

            # modality type embeddings
            BF, lv = x.shape[:2]
            lo = ocr_feats.shape[1]
            self.sp = 1 + lo  # split interval for (cls, text) | (vision)
            type_embeddings = torch.zeros((1 + lv + lo)).to(x).to(torch.long)
            type_embeddings[:1] = 0  # cls token
            type_embeddings[1:1 + lv] = 1  # vision
            type_embeddings[1 + lv:] = 2  # text
            type_embeddings = self.type_embedding(type_embeddings)
            type_embeddings = type_embeddings.expand(BF, -1, -1)

            # input embeddings
            BF = x.shape[0]
            cls_tokens = self.cls_token.expand(BF, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            x = torch.cat((cls_tokens, ocr_feats, x), dim=1)

            # positional embed needs to be tiled for each frame (this does [1,2,3] --> [1,2,3,1,2,3]...)
            cls_embed = self.pos_embed[:, 0, :].unsqueeze(1)
            text_pos_embed = torch.zeros([1, lo, self.patch_embed.embed_dim]).to(cls_embed)
            ...
            total_pos_embed = torch.cat([cls_embed, text_pos_embed, total_pos_embed], dim=1)

            curr_patches = x.shape[1]
            x = x + total_pos_embed[:, :curr_patches] + type_embeddings
            x = self.pos_drop(x)
        ...
        def forward(self, x, ocr_feats):
            x = self.forward_features(x, ocr_feats)
            x = self.head(x)
            return x
